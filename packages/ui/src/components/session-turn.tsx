    ...
    () => {
      ...
      for (let i = index + 1; i < messages.length; i++) {
        ...
        if (item.role === "assistant" && item.parentID === msg.id) {
          const parts = data.store.part[item.id] ?? []
          result.push(item as AssistantMessage)

          console.group(`  🤖 [SessionTurn] Assistant message #${result.length} (id: ${item.id})`)
          console.log("Message Info:", {
            id: item.id,
            parentID: item.parentID,
            hasError: !!item.error,
            completed: !!item.time.completed,
          })

          if (parts.length > 0) {
            console.log("Parts Detail:")
            parts.forEach((p, idx) => {
              const partSummary: Record<string, unknown> = {
                index: idx + 1,
                id: p.id,
                type: p.type,
              }
              ...
              switch (p.type) {
                case "tool": {
                  ...
                  partSummary.tool = tool.tool
                  partSummary.callID = tool.callID
                  partSummary.state = tool.state?.status ?? "unknown"
                  partSummary.metadata = tool.metadata
                  break
                }
                case "reasoning": {
                  const reasoning = p as any
                  partSummary.textPreview = reasoning.text?.substring(0, 100)
                  partSummary.metadata = reasoning.metadata
                  break
                }
                case "text": {
                  const text = p as any
                  partSummary.textPreview = text.text?.substring(0, 100)
                  partSummary.synthetic = text.synthetic
                  partSummary.ignored = text.ignored
                  break
                }
                case "file": {
                  const file = p as any
                  partSummary.filename = file.filename
                  partSummary.mime = file.mime
                  partSummary.url = file.url
                  break
                }
                case "subtask": {
                  const subtask = p as any
                  partSummary.subagentType = subtask.subagent_type
                  partSummary.description = subtask.description
                  partSummary.metadata = subtask.metadata
                  break
                }
                case "snapshot": {
                  const snapshot = p as any
                  partSummary.snapshot = snapshot.snapshot
                  break
                }
                case "patch": {
                  const patch = p as any
                  partSummary.filePath = patch.filePath
                  partSummary.patch = patch.patch?.substring(0, 50) + "..."
                  break
                }
                case "agent": {
                  const agent = p as any
                  partSummary.name = agent.name
                  partSummary.sessionId = agent.sessionId
                  break
                }
                case "retry": {
                  const retry = p as any
                  partSummary.message = retry.message
                  partSummary.attempt = retry.attempt
                  partSummary.next = new Date(retry.next).toISOString()
                  break
                }
              }

              console.log(`    Part ${idx + 1}:`, partSummary)
            })
          } else {
            console.warn("    ⚠️ No parts found for this message")
          }
          console.groupEnd()
        }
      }
